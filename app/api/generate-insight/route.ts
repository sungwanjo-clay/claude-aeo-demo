...
async function generateInsight(req?: Request) {
  ...
  for (const r of responses) {
    ...
    const clayCited = domains.some((d: string) => typeof d === 'string' && d.toLowerCase().includes('anthropic'))
  ...
  const anthropicRes = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 500,
      system:
        `You are an analyst reviewing AI visibility data for Anthropic (anthropic.com). ` +
        `You have 14 days of daily metrics showing how often Anthropic is mentioned, ` +
